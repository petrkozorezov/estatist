...
-type functor()          :: {atom(), atom(), atom()} | fun(() -> any()).
...
-spec tc_update(throw | quite, metric_id(), functor()) -> any().
tc_update(throw, MetricID, Fun) ->
    {T, V} = tc(Fun),
...
-spec select_all() -> select_results().
select_all() ->
    select([]).

-spec select(select_query()) -> select_results().
select(QueryPL) ->
...
-spec measure_call_time(functor(), {time | count, pos_integer()}) -> basho_metrics_nifs:histogram_stats().
